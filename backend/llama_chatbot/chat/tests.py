...
class ChatViewsTestCase(TestCase):
    ...
    def test_get_thread_messages(self):
        ...
        ChatMessage.objects.create(thread=self.thread, sender='bot', content='Hi there!')
        
        response = self.client.get(reverse('get_thread_messages', args=[self.thread_id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_chat_with_model(self):
        response = self.client.post(reverse('chat_with_model', args=[self.thread_id]), data=json.dumps({'message': 'Hello'}), content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_thread_title(self):
        response = self.client.put(reverse('update_thread_title', args=[self.thread_id]), data=json.dumps({'title': 'New Title'}), content_type='application/json')
        self.assertEqual(response.status_code, 200)
    ...
    def test_invalid_message(self):
        response = self.client.post(reverse('chat_with_model', args=[self.thread_id]), data=json.dumps({'message': ''}), content_type='application/json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(json.loads(response.content)['error'], 'No message provided')

    def test_invalid_json(self):
        response = self.client.post(reverse('chat_with_model', args=[self.thread_id]), data='invalid_json', content_type='application/json')
        self.assertEqual(response.status_code, 400)
